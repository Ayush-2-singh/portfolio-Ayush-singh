...
import { FiEdit2, FiTrash2, FiX, FiSave, FiLock, FiEye, FiEyeOff } from 'react-icons/fi'
        ...
        {!isUnlocked ? (
          <div className="max-w-sm mx-auto text-center py-8">
            <div className="sepia-card p-8 rounded-lg">
              <FiLock className="text-ember text-4xl mx-auto mb-4" />
              <p className="font-mono text-dust-500 text-xs uppercase tracking-widest mb-6">
                This journal is sealed. Enter the code to ride in.
              </p>
              <div className="relative mb-3">
                <input
                  ...
                  className={`w-full p-3 bg-darkwood/80 border rounded font-mono text-dust-300 placeholder-dust-600 focus:outline-none transition-colors text-sm pr-10 ${
                    passwordError ? 'border-blood-500' : 'border-saddle-700/50 focus:border-ember/60'
                  }`}
                />
                <button
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-3 top-3.5 text-dust-500 hover:text-ember transition-colors"
                >
                  {showPassword ? <FiEyeOff size={14} /> : <FiEye size={14} />}
                </button>
              </div>
              {passwordError && (
                <p className="font-mono text-blood-400 text-xs mb-3">Wrong code, stranger.</p>
              )}
              <button
                onClick={unlock}
                className="w-full py-3 bg-blood-600 hover:bg-blood-500 text-parchment font-mono uppercase tracking-widest text-sm rounded transition-all border border-blood-500/30"
              >
                Unlock Journal
              </button>
            </div>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              ...
              <div className="sepia-card p-6 rounded-lg">
                ...
                <button
                  ...
                  className="w-full mt-3 py-2 border border-saddle-600 text-dust-500 font-mono text-xs uppercase tracking-wider rounded hover:border-blood-500 hover:text-blood-400 transition-colors"
                >
                  Lock Journal
                </button>
              </div>
